...
def setup_ranger_plugin(component_select_name, service_name,
  ...
  file_path = format('/usr/lib/ranger-{service_name}-plugin/install.properties')
  ...
  cmd_env = {'JAVA_HOME': java_home, 'PWD': format('/usr/lib/ranger-{service_name}-plugin'), 'PATH': format('/usr/lib/ranger-{service_name}-plugin')}
