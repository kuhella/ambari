...
import ENV from 'gpdb-view/config/environment';
...
export default {
  ...
  getNamespace() {
    let version = '1.0.0',
      instanceName = 'GPDB',
      apiPrefix = '/api/v1/views/GPDB/versions/',
    ...
    if (params[params.length - 3] === 'GPDB') {
      version = params[params.length - 2];
      instanceName = params[params.length - 1];
    }

    let gpdbViewInstanceURL = `${apiPrefix}${version}${instancePrefix}${instanceName}`;
    return ENV.environment === 'test' ? ENV.apiURL : gpdbViewInstanceURL;
  },
