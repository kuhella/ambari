...
**Известные проблемы:**
- Apache Atlas: Возможно появление предупреждения (Warning) о недоступности сервиса;

- Apache Ozzie: Возможно возникновение ошибки при проверке сервиса (Service Check) из-за недостатка ресурсов, возникает как правило при установке множества сервисов на одном узле;

- Apache Metrics: Некоторые метрики могут отображаться некорретно;

- Apache Giraph: В некотрых случаях остановка сервиса может завершаться некрректно;
