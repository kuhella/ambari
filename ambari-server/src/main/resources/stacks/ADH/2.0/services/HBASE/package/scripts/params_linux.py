...
if stack_version_formatted and check_stack_feature(StackFeature.ROLLING_UPGRADE, stack_version_formatted):
  daemon_script = format('{stack_root}/hbase/bin/hbase-daemon.sh')
  region_mover = format('{stack_root}/lib/hbase/bin/region_mover.rb')
  region_drainer = format('{stack_root}/lib/hbase/bin/draining_servers.rb')
  hbase_cmd = format('{stack_root}/lib/hbase/bin/hbase')

  hbase_max_direct_memory_size  = default('configurations/hbase-env/hbase_max_direct_memory_size', None)

  daemon_script=format("{stack_root}/hbase/bin/hbase-daemon.sh")
  region_mover = format("{stack_root}/hbase/bin/region_mover.rb")
  region_drainer = format("{stack_root}/hbase/bin/draining_servers.rb")
  hbase_cmd = format("{stack_root}/hbase/bin/hbase")
...
if enable_ranger_hbase:
  ...
  if has_ranger_admin and stack_supports_ranger_audit_db:
    ...
    driver_curl_target = format("{stack_root}/hbase/lib/{jdbc_jar_name}") if stack_supports_ranger_audit_db else None
    previous_jdbc_jar = format("{stack_root}/hbase/lib/{previous_jdbc_jar_name}") if stack_supports_ranger_audit_db else None
    sql_connector_jar = ''
