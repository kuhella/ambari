...
if OSCheck.is_windows_family():
  ...
  hive_server_conf_dir = "/etc/hive/conf/conf.server"
  hive_server_interactive_conf_dir = "/etc/hive2/conf/conf.server"

  webhcat_conf_dir = format("/etc/webhcat/conf")
  hive_home_dir = format("/usr/lib/hive")
  hive_conf_dir = format("/etc/hive/conf")
  hive_client_conf_dir = format("/etc/hive/conf")
  ...
  if check_stack_feature(StackFeature.HIVE_WEBHCAT_SPECIFIC_CONFIGS, stack_version_formatted_major):
    ...
    webhcat_conf_dir = format("/etc/webhcat")
  ...
  if check_stack_feature(StackFeature.HIVE_SERVER_INTERACTIVE, stack_version_formatted_major):
    hive_server_interactive_conf_dir = format("/etc/hive2/conf/conf.server")
