...
spark_history_server_start = format("/usr/lib/spark/sbin/start-history-server.sh")
spark_history_server_stop = format("/usr/lib/spark/sbin/stop-history-server.sh")

spark_thrift_server_start = format("/usr/lib/spark/sbin/start-thriftserver.sh")
spark_thrift_server_stop = format("/usr/lib/spark/sbin/stop-thriftserver.sh")
spark_logs_dir = format("/var/log/spark/")

spark_submit_cmd = format("/usr/lib/spark/bin/spark-submit")
spark_smoke_example = "org.apache.spark.examples.SparkPi"
spark_service_check_cmd = format(
  "{spark_submit_cmd} --class {spark_smoke_example}  --master yarn-cluster  --num-executors 1 --driver-memory 256m  --executor-memory 256m   --executor-cores 1  /usr/lib/spark/examples/jars/spark-examples*.jar 1")
